...
/** 기존 링크 호환용 경로. 과제 홈의 발표자료 단계로 연결한다. */
export default function PresentationRedirectScreen() {
  const params = useLocalSearchParams<{ projectId?: string }>();
  const projectId = Array.isArray(params.projectId) ? params.projectId[0] : params.projectId;
  const theme = useTheme();

  useEffect(() => {
    if (projectId) router.replace(`/projects/${projectId}?projectView=home&flowStep=presentation` as Href);
  }, [projectId]);

  if (!projectId) {
    return (
      <ThemedView style={[styles.center, { backgroundColor: theme.background }]}>
        <ThemedText type="smallBold">과제 홈에서 발표자료를 만들어 주세요.</ThemedText>
  ...
  return (
    <ThemedView style={[styles.center, { backgroundColor: theme.background }]}>
      <ActivityIndicator />
      <ThemedText type="small" themeColor="textSecondary">발표자료 단계로 이동하는 중…</ThemedText>
    </ThemedView>
  );
}
