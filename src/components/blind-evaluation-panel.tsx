  ...
  return (
    <ThemedView type="surface" style={[styles.container, { borderColor: theme.border }]}>
      <View style={styles.header}>
        ...
        <View style={[styles.anonymousBadge, { backgroundColor: theme.successSoft }]}><ThemedText type="captionStrong" style={{ color: theme.success }}>익명 평가</ThemedText></View>
      </View>
      {ideas.map((idea) => (
            ...
            <View key={criterion} style={styles.criterionRow}>
              ...
              <View style={styles.scores}>
                {scoreOptions.map((option) => {
                  ...
                  return <Pressable key={option} accessibilityRole="radio" accessibilityState={{ selected }} onPress={() => setScore(idea.id, criterion, Number(option))} style={[styles.score, { borderColor: theme.border }, selected && { backgroundColor: theme.primary, borderColor: theme.primary }]}><ThemedText type="captionStrong" style={selected ? styles.selectedText : undefined}>{option}</ThemedText></Pressable>;
                })}
              </View>
            </View>
          ))}
      ...
      <View style={styles.footer}>
        {saved ? <ThemedText type="small" style={{ color: theme.success }}>평가가 저장되었습니다.</ThemedText> : null}
        <Pressable disabled={isSaving} onPress={() => void submit()} style={[styles.submit, { backgroundColor: theme.primary }, isSaving && styles.disabled]}>{isSaving ? <ActivityIndicator color="#fff" /> : <ThemedText type="smallBold" style={styles.white}>평가 제출</ThemedText>}</Pressable>
      </View>
    </ThemedView>
  );
}
...
const styles = StyleSheet.create({
  container: { borderWidth: 1, borderRadius: Radius.large, padding: Spacing.three, gap: Spacing.three },
  ...
  anonymousBadge: { alignSelf: 'flex-start', borderRadius: Radius.pill, paddingHorizontal: Spacing.two, paddingVertical: Spacing.one },
  ...
  score: { width: 34, height: 34, borderWidth: 1, borderRadius: Radius.small, alignItems: 'center', justifyContent: 'center' },
  selectedText: { color: '#fff' },
  footer: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: Spacing.two },
  submit: { minHeight: ControlHeight.button, borderRadius: Radius.medium, justifyContent: 'center', paddingHorizontal: Spacing.three },
  white: { color: '#fff' },
