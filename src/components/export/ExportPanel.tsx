...
export function ExportPanel({ data }: { data: ExportData }) {
  ...
  const actions = [
    ...
    { label: '사업계획서 PDF 다운로드', run: () => exportToPdf(data, 'business-plan'), message: Platform.OS === 'web' ? '인쇄 창에서 “PDF로 저장”을 선택해 주세요.' : fileMessage },
    { label: '최종보고서 PDF 다운로드', run: () => exportToPdf(data, 'final-report'), message: Platform.OS === 'web' ? '인쇄 창에서 “PDF로 저장”을 선택해 주세요.' : fileMessage },
  ...
  return (
    <ThemedView type="backgroundElement" style={[styles.panel, { borderColor: theme.border }]}>
      ...
      <ThemedText type="small" themeColor="textSecondary">
        선정 아이디어와 MVP 결과를 저장합니다. 발표자료를 생성하거나 부분 재작성하면 최신 내용이 내보내기에 바로 반영됩니다.
      </ThemedText>
